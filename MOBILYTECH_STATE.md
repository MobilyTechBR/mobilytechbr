...
- Pendencias seguintes: publicar o endpoint com `dryRun`, testar `/api/register-sale` com token correto sem gravar venda, verificar `/favicon.ico` no dominio oficial, sincronizar handoff/copies e investigar crash/fechamento do Codex por ultimo. Wix Headless/Auth real fica opcional enquanto nao houver fluxo completo de segredo OAuth seguro.
