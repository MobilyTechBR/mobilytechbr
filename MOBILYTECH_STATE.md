...
- Fechamento adicional 2026-06-16: os 11 modelos de e-mails transacionais foram regenerados a partir de `docs/google-apps-script/mobilytech-pos-venda.gs` e enviados como testes reais para a conta MobilyTech/Gmail, com assuntos `[TESTE CLIENTE]` e `[TESTE VENDEDOR]`; os 11 envios foram confirmados em `SENT`. Depois do feedback do usuario, os e-mails de vendedor ganharam tema proprio em azul mais escuro, faixa superior discreta e CTAs/blocos no mesmo tom, mantendo a estrutura; 5 novos testes `[TESTE VENDEDOR - AJUSTE COR]` foram enviados ao Gmail.
- Fechamento adicional 2026-06-16: `favicon.ico`, `favicon.png` e `apple-touch-icon.png` foram adicionados tambem na raiz publica do Vercel para cobrir navegadores que solicitam `/favicon.ico` direto; o HTML oficial ja apontava para `assets/favicon.png`.
- `ADMIN_WRITE_TOKEN`: o usuario informou que colou a env var em Production na Vercel e fez redeploy. Producao passou a retornar 401 com token errado em vez de 501, confirmando que a env var esta ativa. Foi adicionado `dryRun`/`mode=auth-check` a `api/register-sale.js` para validar o token correto sem registrar venda real. Commit `99106c0` deployou na Vercel como `dpl_BHy7ttr5FocwThk5wHazzgzwQ32M`; `https://www.mobilytech.com.br/api/register-sale?qa=99106c0-authcheck` retornou 200 com `authenticated:true` e `dryRun:true` usando o token real, sem registrar venda.
- Login/headless: nao criar OAuth App Wix nem botao social real enquanto nao houver caminho seguro para gravar imediatamente o segredo em env vars. A pagina `fase2/minha-conta.html` continua como consulta/atendimento seguro de pedido, nao login fake.
- Producao validada 2026-06-16: `www.mobilytech.com.br`, `/fase2/ofertas.html`, `/fase2/minha-conta.html`, `/favicon.ico`, `/favicon.png` e `/apple-touch-icon.png` retornaram 200 via Vercel com cache-buster `qa=99106c0`.
- Painel de conteudo visual: `private/admin/index.html` permite editar `data/site-content.json`, baixar fallback e agora chamar `/api/update-site-content`. A rota exige admin, valida JSON e so grava no GitHub se `GITHUB_CONTENT_WRITE_TOKEN` estiver configurado como segredo da Vercel.
- Dropshipping/fornecedor: QA controlado em `docs/qa/supplier-fulfillment-control-2026-06-16/qa.json` validou 51 produtos de fornecedor ativos e `FornecedorItens` completo. Para origem internacional, `lib/fulfillment-shipping.js` prioriza `supplierSearchUrl`/fornecedor base no e-mail do vendedor.
- Publicacao site-content: deploy inicial `3768d2a` falhou por adicionar uma 13a funcao Vercel; commit `848b979` consolidou `/api/update-site-content` dentro de `api/account.js?action=update-site-content`, deploy `dpl_7cc7YG3CYrMrXNFiorUgUmjV4t7c` ficou `READY` e QA production passou em `docs/qa/production-site-content-writer-2026-06-16-848b979/qa.json`.
- Crash/fechamento Codex: investigacao final apontou APPCRASH do Opera GX `132.0.5905.43` em `opera_browser.dll` (`0xc0000005`) e historico `LiveKernelEvent`/watchdog GPU; Crashpad do Codex tem dumps recentes, mas sem APPCRASH textual do Codex nas ultimas horas. Mitigacao operacional: evitar Opera GX/Browser interno para OAuth/Apps Script/tarefas longas e preferir conectores/API/shell/HTTP.
- Pendencias seguintes: sincronizar handoff/copies e tratar eventual publicacao do Apps Script vivo com cuidado para nao sobrescrever a codificacao mensal existente. Wix Headless/Auth real fica opcional enquanto nao houver fluxo completo de segredo OAuth seguro.
