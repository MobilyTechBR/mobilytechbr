...
function normalizeDropifyProduct(product = {}) {
  ...
  return {
    ...
    description: compactText(product.description || product.descriptions || "", 500),
    ...
    wholesalePrice: moneyFromPossibleCents(product.wholesalePrice),
    suggestedRetailPrice: moneyFromPossibleCents(product.suggestedRetailPrice),
    measurements: {
...
async function quoteDropifySupplierFreight(products = [], postalCode) {
  ...
  return freightOptionsArray(data)
    .map((option) => {
      ...
      return {
        id: `dropify:${option.id || option.code || option.method || name}`,
        name,
