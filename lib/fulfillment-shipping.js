...
function isSupplierFulfilled(product) {
  ...
  const fulfillmentMode = normalizeText(product?.fulfillmentMode || product?.purchaseMode || "");
  const shippingMode = normalizeText(product?.shipping?.mode || "");
  return Boolean(
...
function supplierFulfillmentItem(product) {
  const freight = supplierShippingPrice(product);
  return {
    productId: product?.id || "",
    title: product?.title || "",
    supplierPlatform: product?.supplierPlatform || product?.marketplace?.name || "Fornecedor parceiro",
    ...
    customerShippingPrice: freight.price,
...
function supplierQuote(products = []) {
  const items = products.map(supplierFulfillmentItem);
  ...
  return {
    id: "supplier-manual",
    name: "Envio nacional com rastreio",
    company: "Entrega parceira",
    price,
    deliveryTime,
    recommended: true,
...
function combineShippingQuotes(melhorEnvioQuotes = [], supplier) {
  return melhorEnvioQuotes.map((quote) => ({
    ...quote,
    id: `mixed:${quote.id}`,
    name: `${quote.name} + envio parceiro`,
...
async function buildShippingQuotes(products, postalCode, { quoteMelhorEnvio, aggregateShippingProduct }) {
  const split = splitFulfillmentProducts(products);
  const supplier = split.supplier.length ? supplierQuote(split.supplier) : null;
  ...
  if (supplier && physicalResult) {
    return {
      provider: "mixed",
      quotes: combineShippingQuotes(physicalResult.quotes, supplier),
      supplier,
...
async function resolveShippingSelection(products, shipping, helpers) {
  ...
  if (!shipping || !shipping.postalCode || !shipping.serviceId) {
    if (split.supplier.length) {
      const error = new Error("Calcule e selecione o envio para produtos enviados por fornecedor parceiro.");
...
function formatFulfillmentItems(items = []) {
  return items.map((item) => [
    `Produto: ${item.title}`,
    `ID: ${item.productId}`,
    `Fornecedor: ${item.supplierPlatform}`,
