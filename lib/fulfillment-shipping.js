...
function allowsMultiplePhysicalUnits(product) {
  const category = normalizeText(product?.category || product?.type || "");
  return Boolean(
    product?.allowQuantity === true ||
...
function validateUniquePhysicalCheckoutItems(checkoutItems = []) {
  ...
  checkoutItems.forEach((item) => {
    ...
    const allowMultiple = allowsMultiplePhysicalUnits(product);
    ...
    if (!allowMultiple && seen.has(productId)) {
...
function isInternalProcurementProduct(product) {
  const category = normalizeText(product?.category || product?.type || "");
  const fulfillmentMode = normalizeText(product?.fulfillmentMode || product?.purchaseMode || product?.fulfillment?.mode || "");
  return Boolean(
    product?.madeToOrder === true ||
    category === "sob-encomenda" ||
    category === "sob encomenda" ||
    category === "encomenda" ||
    fulfillmentMode.includes("made-to-order") ||
    fulfillmentMode.includes("preorder") ||
    fulfillmentMode.includes("mobilytech-preorder")
  );
}
...
function formatBrl(value) {
  ...
  return number !== null ? `R$ ${toMoney(number).toFixed(2).replace(".", ",")}` : "Nao informado";
}

function formatPercent(value) {
  const number = parseMoneyNumber(value);
  return number !== null ? `${toMoney(number).toFixed(2).replace(".", ",")}%` : "Nao informado";
}

function internalProcurementItem(product) {
  ...
  const supplierCost = firstMoneyNumber(product?.supplierCost, product?.costPrice);
  const inboundShippingCost = firstMoneyNumber(product?.inboundShippingCost, product?.inboundFreightCost, product?.shipping?.inboundShippingCost, product?.shipping?.inboundFreightCost) || 0;
  const computedBaseCost = supplierCost !== null ? toMoney(supplierCost + inboundShippingCost) : null;
  const baseCost = firstMoneyNumber(product?.baseCost, computedBaseCost);
  const salePrice = firstMoneyNumber(product?.salePrice, product?.price);
  const saleTotal = salePrice !== null ? toMoney(salePrice * quantity) : null;
  const supplierCostTotal = supplierCost !== null ? toMoney(supplierCost * quantity) : null;
  const inboundShippingTotal = toMoney(inboundShippingCost * quantity);
  const baseCostTotal = baseCost !== null ? toMoney(baseCost * quantity) : null;
  const profit = saleTotal !== null && baseCostTotal !== null ? toMoney(saleTotal - baseCostTotal) : null;
  const marginPercent = saleTotal && profit !== null ? toMoney((profit / saleTotal) * 100) : null;
  const markupPercent = baseCostTotal && profit !== null ? toMoney((profit / baseCostTotal) * 100) : null;
  const configuredMarginPercent = firstMoneyNumber(product?.targetMarginPercent, product?.marginPercent);
  const sourceNoteText = typeof product?.sourceNotes === "string"
    ? product.sourceNotes
    : (sourceNotes?.supplierInstruction || sourceNotes?.note || "");
  ...
  return {
    ...
    saleUnitPrice: salePrice !== null ? toMoney(salePrice) : null,
    saleTotal,
    supplierUnitCost: supplierCost !== null ? toMoney(supplierCost) : null,
    supplierCostTotal,
    inboundShippingUnit: toMoney(inboundShippingCost),
    inboundShippingTotal,
    baseUnitCost: baseCost !== null ? toMoney(baseCost) : null,
    baseCostTotal,
    profit,
    marginPercent,
    markupPercent,
    configuredMarginPercent,
    procurementBusinessDays: firstMoneyNumber(product?.procurementBusinessDays, product?.shipping?.procurementBusinessDays),
    handlingBusinessDays: firstMoneyNumber(product?.handlingBusinessDays, product?.shipping?.handlingBusinessDays),
    sourceNotes: compactText(sourceNoteText, 260)
  };
}

function procurementItemsForProducts(products = []) {
  return products
    .filter((product) => product && isInternalProcurementProduct(product))
    .map(internalProcurementItem);
}

function procurementItemsForIds(products, productIds = [], quantitiesById = {}) {
  const wanted = new Set(productIds.map((id) => String(id || "").trim()).filter(Boolean));
  return products
    .filter((product) => wanted.has(String(product.id)) && isInternalProcurementProduct(product))
    .map((product) => internalProcurementItem({
      ...product,
      quantity: quantitiesById[String(product.id)] || product.quantity || 1
    }));
}
...
function supplierAddOnCartError(products = [], options = {}) {
  ...
  const error = new Error(`${titles} tem frete proporcionalmente alto quando comprado sozinho. Adicione outros produtos sob encomenda ate pelo menos R$ ${minSubtotal.toFixed(2).replace(".", ",")} para recalcular o envio em combo.`);
  error.statusCode = 409;
...
function formatProcurementItems(items = []) {
  return items.map((item) => [
    `Produto: ${item.title}`,
    `ID: ${item.productId}`,
    item.selectedVariant?.label ? `Variacao: ${item.selectedVariant.label}` : "",
    `Qtd: ${item.quantity || 1}`,
    `Valor da venda: ${formatBrl(item.saleUnitPrice)} un. | total ${formatBrl(item.saleTotal)}`,
    `Custo do produto para MobilyTech: ${formatBrl(item.supplierUnitCost)} un. | total ${formatBrl(item.supplierCostTotal)}`,
    `Frete estimado ate a MobilyTech: ${formatBrl(item.inboundShippingUnit)} un. | total ${formatBrl(item.inboundShippingTotal)}`,
    `Custo total estimado MobilyTech: ${formatBrl(item.baseCostTotal)}`,
    `Lucro bruto estimado: ${formatBrl(item.profit)}`,
    `Margem bruta estimada: ${formatPercent(item.marginPercent)} sobre a venda | ${formatPercent(item.markupPercent)} sobre o custo`,
    item.configuredMarginPercent !== null && item.configuredMarginPercent !== undefined ? `Margem configurada no painel: ${formatPercent(item.configuredMarginPercent)} sobre o custo` : "",
    `Fornecedor/origem: ${item.supplierPlatform || "Nao informado"}`,
    `Link para comprar: ${item.supplierUrl || "Nao informado"}`,
    item.supplierBackupUrl ? `Backup: ${item.supplierBackupUrl}` : "",
    item.procurementBusinessDays || item.handlingBusinessDays
      ? `Prazo interno estimado: fornecedor ate MobilyTech ${item.procurementBusinessDays || "?"} dia(s) + despacho MobilyTech ${item.handlingBusinessDays || "?"} dia(s)`
      : "",
    item.sourceNotes ? `Observacao interna: ${compactText(item.sourceNotes, 220)}` : ""
  ].filter(Boolean).join(" | ")).join("\n");
}
