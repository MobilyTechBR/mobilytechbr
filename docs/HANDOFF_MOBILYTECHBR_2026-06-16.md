...
A proxima IA deve comecar corrigindo o erro do gerador em `scripts/build_phase2_ibuy_style.py` causado por `${quantity}` dentro de f-string Python. Depois deve regenerar o site, testar as alteracoes de carrinho/frete/quantidade ja aplicadas nas APIs/libs, atualizar o ledger e seguir a fila: painel, Apps Script/Planilha OLX, e-mails, Wix/headless/dominio, login seguro, crosscheck final e publicacao.
